...
def bakeAction(frame_start,
    ...
    def poseFrameInfo(obj, do_visual_keying):
        matrix = {}
    ...
    for f in frame_range:
        ...
        scene.update()
        if do_pose:
            pose_info.append(poseFrameInfo(obj, do_visual_keying))
        if do_object:
            obj_info.append(objFrameInfo(obj, do_visual_keying))
    ...
    if do_pose:
        for name, pbone in obj.pose.bones.items():
            ...
            # Force the use of quaternions if option is enabled
            rotation_mode_backup = pbone.rotation_mode
            if use_quaternions and pbone.rotation_mode != 'QUATERNION':
